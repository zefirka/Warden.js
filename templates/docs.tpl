...
<h3 id="streams_info">State and values:</h3>
...
<h2 id="lock">.lock</h2>
<p class='d-synopsis'>Synopsis: <code>stream.lock()</code>.</p>
<p class='d-description'>Description: Locks stream and all stream's children (inherited streams).</p>

<h2 id="lockt">.lockThis</h2>
<p class='d-synopsis'>Synopsis: <code>stream.lockThis()</code>.</p>
<p class='d-description'>Description: Locks only given stream.</p>

<hr>
<h2 id="ulock">.unlock</h2>
<p class='d-synopsis'>Synopsis: <code>stream.unlock()</code>.</p>
<p class='d-description'>Description: Unlocks stream and all stream's children (inherited streams).</p>

<h2 id="ulockt">.unlockThis</h2>
<p class='d-synopsis'>Synopsis: <code>stream.unlockThis()</code>.</p>
<p class='d-description'>Description: Locks only given stream.</p>
...
<hr class='bhr'>       
<h2 id="host">Warden.Host</h2>
<p class='d-synopsis'>Synopsis: <code>Warden.Host([context]);</code>.</p>
<p class='d-description'>Description: module which hosts streams on given context</p> 
<h4>Usage:</h4>
<p>Hosts makes streams faster and also they bind streams to the context by default. Sometimes we generate a lot of streams from one initial source (e.g. clicks on elements, scrolls of window etc). To not overload elements by bunch of listeners Warden provides hosts. Hosts are simple storage of active streams, and when events happens host's listener runs all active streams associated to the given host.</p>
<pre><code class='javascript'>var stream = Warden.Stream();
...
<h2>.eval</h2>
<p class='d-synopsis'>Synopsis: <code>host.eval(data, [context])</code>.</p>
<p class='d-description'>Description: evaluates host's active streams with given data. If context is not setted uses host's context</p> 

<h2>.push</h2>
<p class='d-synopsis'>Synopsis: <code>host.push(stream);</code>.</p>
<p class='d-description'>Description: push stream to the host's active streams query.</p> 

<h2>.pop</h2>
<p class='d-synopsis'>Synopsis: <code>host.pop(stream)</code>.</p>
<p class='d-description'>Description: removes stream from hosts's active streams query.</p> 

<h2>.newStream</h2>
<p class='d-synopsis'>Synopsis: <code>host.newStream()</code>.</p>
...
<h3>Usage:</h3>
<p>Formulas implement reactive calculations in Warden. In pure JS when we assign value to the variable we just say that variable literaly refers to the given value. When we change value, we don't change the references of value, it's obvious:</p>
...
<p>Warden.Formula - is a way to say that values of given variable not just refers to other value but depends on values of other variables. It implemented with streams, so <code>Warden.Formula</code> takes and returns streams object.</p>
<pre><code class='javascript'>var a = Warden(10); //stream 
var b = Warden(20);
